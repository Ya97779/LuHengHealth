...
//  蓝牙协议解析器 v1.2
//  负责解析蓝牙设备传输的数据协议
//  支持心率、血氧、步数、电池电压数据的解析

import Foundation

// MARK: - 解析结果结构体
/// 解析后的数据结构，包含心率、血氧值、电池电压、步数和版本号
...
class BLEProtocolParser {
    ...
    private static let stepCountHighIndex = 6   // 步数高8位位置 (v1.2新增)
    private static let stepCountLowIndex = 7    // 步数低8位位置 (v1.2新增)
    private static let firmwareVersionIndex = 8  // 固件版本号位置 (v1.2新增)
    private static let frameFooter1Index = 15   // 帧尾1位置 (v1.2变更)
    private static let frameFooter2Index = 16   // 帧尾2位置 (v1.2变更)

    /// v1.2 数据包总长度
    static let v12DataLength = 17
    ...
    func parseFFE4Data(_ data: Data) -> BLEHealthData {
        ...
        // 检查数据长度是否足够（v1.2完整数据包应该是17字节）
        guard data.count >= BLEProtocolParser.v12DataLength else {
        ...
        // 检查帧头帧尾 (v1.2: 帧尾在Byte 15-16)
        guard data[0] == BLEProtocolParser.frameHeader1 &&
              data[1] == BLEProtocolParser.frameHeader2 &&
              data[BLEProtocolParser.frameFooter1Index] == BLEProtocolParser.frameFooter1 &&
              data[BLEProtocolParser.frameFooter2Index] == BLEProtocolParser.frameFooter2 else {
            return BLEHealthData(
                heartRate: nil,
                bloodOxygen: nil,
                batteryVoltage: nil,
        ...
        // 提取步数 (v1.2新增: 高字节在前，低字节在后)
        let stepCountHigh = Int(data[BLEProtocolParser.stepCountHighIndex])
        let stepCountLow = Int(data[BLEProtocolParser.stepCountLowIndex])
        let stepCount = (stepCountHigh << 8) + stepCountLow

        // 提取固件版本号 (v1.2新增: 例如12代表1.2，24代表2.4)
        let firmwareVersion = Int(data[BLEProtocolParser.firmwareVersionIndex])
